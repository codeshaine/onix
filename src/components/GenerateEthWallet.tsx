...
// export default function GenerateEthWallet() {
//   const { mnemonic, ethWallets, SetEthWallets } = useAppContext();

//   async function generateWallet() {
//     const seed = await mnemonicToSeed(mnemonic);
//     const derivePath = `m/44'/60'/${ethWallets.length}'/0'`;
...
export default function GenerateEthWallet() {
  ...
  async function generateWallet() {
    ...
    try {
      const seed = await mnemonicToSeed(mnemonic);
      const derivePath = `m/44'/60'/${ethWallets.length}'/0/0`; // Fixed the syntax error
      const hdNode = HDNodeWallet.fromSeed(seed);
      const child = hdNode.derivePath(derivePath);
      const privateKey = child.privateKey;
      const wallet = new Wallet(privateKey);
  ...
  const copyToClipboard = async (text: string, key: string): Promise<void> => {
    try {
      ...
      setCopiedItems((prev: CopiedItemsState) => ({ ...prev, [key]: true }));
      setTimeout(() => {
        setCopiedItems((prev: CopiedItemsState) => ({ ...prev, [key]: false }));
      }, 2000);
    } catch (err) {
  ...
  const removeWallet = (indexToRemove: number) => {
    setEthWallets(ethWallets.filter((_, index) => index !== indexToRemove));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Button
        ...
        className="w-full h-12 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
      >
        {isGenerating ? (
      ...
      <Card className="bg-slate-800/50 backdrop-blur-sm border-slate-700/50">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            >
              {ethWallets.length} Wallet{ethWallets.length !== 1 ? "s" : ""}
            </Badge>
          </div>
        </CardHeader>
        <CardContent className="pt-0">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            copyToClipboard(wallet.address, `address-${idx}`);
                          }}
